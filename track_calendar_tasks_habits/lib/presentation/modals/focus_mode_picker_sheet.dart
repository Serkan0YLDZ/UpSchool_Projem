...
Future<FocusSection?> showFocusModePickerSheet(BuildContext context) async {
  ...
  final calendarEventProvider = context.read<CalendarEventProvider>();
  ...
  await calendarEventProvider.loadEvents(selectedDate);
  ...
  if (!context.mounted) return null;
  await streakProvider.loadForHabits(habitProvider.habits, todayYmd);
  if (!context.mounted) return null;
